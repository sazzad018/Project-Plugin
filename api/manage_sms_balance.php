...
// 1. Auto-create table if it doesn't exist (Global Store)
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($license_key) {
        // Fetch specific license balance
        $clean_key = $conn->real_escape_string($license_key);
        $res = $conn->query("SELECT sms_balance FROM licenses WHERE license_key = '$clean_key'");
        if ($res && $res->num_rows > 0) {
            echo json_encode(["balance" => (int)$res->fetch_assoc()['sms_balance']]);
        } else {
            echo json_encode(["balance" => 0, "error" => "Invalid License"]);
        }
    } else {
        // Fetch Global Admin Balance
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Only used by Admin Dashboard to update global balance
